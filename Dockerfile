...
RUN sed -i -E "s#[^/]+.ubuntu.com#mirrors.aliyun.com#g" /etc/apt/sources.list \
    && apt-get update \
    && apt-get -y install php7.2 php7.2-mbstring php7.2-xml php7.2-curl php7.2-ds zsh composer sudo unzip language-pack-zh-hans curl git python3 gettext libxapian30\

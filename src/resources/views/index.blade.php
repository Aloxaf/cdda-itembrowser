...
<p>
<h2>常用物品</h2>
<a href="{{route('item.recipes', array('id'=>'fire')) }}">生火</a>总是没有坏处的。<a href="{{route('item.view', "toolset") }}">瑞士军手</a>可以用来制造大量物品。<br>
<br>
为了修复你的装备和衣服，你可以切割一些
<a href="{{ route('item.materials', 'wood') }}">木质物品</a>
 来获得
 <a href="{{ route('item.view', 'skewer') }}">碎木</a>，
 然后你就可以
 <a href="{{ route('item.craft', 'needle_wood') }}">制作一根木针</a>，
 接着你需要
 <a href="{{ route('item.view', 'thread') }}">缝衣线</a>
 这个可以通过
 <a href="{{ route('item.disassemble', 'rag') }}">拆解一块布条</a>来获得，
 而布条可以通过切割
